...
#' jointly modeling Y and U is desired use \code{adjust_multinom_uc_omc_sel}.
#'
...
#'  where U is the binary unmeasured confounder, X is the binary exposure,
#'  and Y is the binary true outcome. The number of parameters therefore
#'  equals 3.
...
adjust_uc_omc_sel <- function(
  ...
  if (is.null(confounders)) {
    ...
    df <- df %>%
      mutate(
        Ypred = rbinom(n, 1, plogis(y1_0 + y1_x * .data$X +
                                      y1_ystar * .data$Ystar +
                                      y1_c1 * .data$C1)),
        Upred = rbinom(n, 1, plogis(u1_0 + u1_x * .data$X +
                                      u1_y * .data$Ypred)),
        pS = plogis(s1_0 + s1_x * .data$X + s1_ystar * .data$Ystar +
                      s1_c1 * .data$C1)
      )
